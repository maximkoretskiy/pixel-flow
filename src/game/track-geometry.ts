...
export function trackPoint(distance: number, routeLength: number): TrackPoint {
  ...
  if (normalized <= 0.25) {
    return { x: x + width * (normalized / 0.25), y: y + height, angle: 0 };
  }
  if (normalized <= 0.5) {
    return { x: x + width, y: y + height * (1 - (normalized - 0.25) / 0.25), angle: -Math.PI / 2 };
  }
  if (normalized <= 0.75) {
    return { x: x + width * (1 - (normalized - 0.5) / 0.25), y, angle: Math.PI };
  }
  return { x, y: y + height * ((normalized - 0.75) / 0.25), angle: Math.PI / 2 };
}
