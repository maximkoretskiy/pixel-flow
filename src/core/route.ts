...
export function buildRoute(width: number, height: number): Route {
  const rays: Ray[] = [];
  for (let x = 0; x < width; x += 1) rays.push({ edge: 'bottom', index: x });
  for (let y = height - 1; y >= 0; y -= 1) rays.push({ edge: 'right', index: y });
  for (let x = width - 1; x >= 0; x -= 1) rays.push({ edge: 'top', index: x });
  for (let y = 0; y < height; y += 1) rays.push({ edge: 'left', index: y });
